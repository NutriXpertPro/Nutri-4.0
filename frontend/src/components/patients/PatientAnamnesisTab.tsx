                        ...
                        <header className="p-8 md:p-12 border-b border-border/5 bg-linear-to-b from-muted/20 to-transparent relative overflow-hidden">
                            <div className="flex flex-col md:flex-row justify-between items-start gap-8 relative z-10">
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-4 pt-4 border-t border-border/10">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-base font-normal text-foreground">{standardAnamnesis?.nome || patient?.name}</p>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Data do Relatório</p>
                                            <p className="text-base font-normal text-foreground">{standardAnamnesis?.updated_at ? new Date(standardAnamnesis.updated_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' }) : '--'}</p>
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20 px-3 py-0.5 rounded-full text-[10px] ">
                                                {standardAnamnesis?.objetivo || "Não especificado"}
                                            </Badge>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-[10px] text-muted-foreground uppercase tracking-widest">ID Registro</p>
                                            <p className="text-xs font-mono text-muted-foreground font-normal">#ANM-{standardAnamnesis?.id?.toString().padStart(4, '0') || '----'}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="hidden lg:flex flex-col items-end gap-3 text-right">
                                    ...
                                    <div>
                                        <p className="text-sm  text-foreground">{patient?.birth_date ? calculateAge(patient.birth_date) : '--'} Anos</p>
                                    ...
                                    <div className="p-8 md:p-12 rounded-[2.5rem] border border-border/10 bg-white/40 dark:bg-zinc-900/10 shadow-sm backdrop-blur-xs relative overflow-hidden">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-16 relative z-10">
                                            <ClinicalSection title="1. Identificação" icon={User}>
                                                <DataField label="Nome Completo" value={standardAnamnesis.nome} />
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Idade" value={standardAnamnesis.idade ? `${standardAnamnesis.idade} anos` : "--"} />
                                                    <DataField label="Sexo" value={standardAnamnesis.sexo} />
                                                </div>
                                                <DataField label="Data de Nascimento" value={standardAnamnesis.nascimento ? new Date(standardAnamnesis.nascimento).toLocaleDateString('pt-BR') : "--"} />
                                                <DataField label="Profissão" value={standardAnamnesis.profissao} />
                                                <DataField label="E-mail" value={standardAnamnesis.email} />
                                                <DataField label="Telefone" value={standardAnamnesis.telefone} />
                                            </ClinicalSection>

                                            <ClinicalSection title="2. Rotina & Estilo de Vida" icon={Moon}>
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Acorda" value={formatTime(standardAnamnesis.hora_acorda)} />
                                                    <DataField label="Dorme" value={formatTime(standardAnamnesis.hora_dorme)} />
                                                </div>
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Dificuldade p/ Dormir" value={standardAnamnesis.dificuldade_dormir ? "Sim" : "Não"} badge={standardAnamnesis.dificuldade_dormir ? "red" : "green"} />
                                                    <DataField label="Acorda à Noite" value={standardAnamnesis.acorda_noite ? "Sim" : "Não"} badge={standardAnamnesis.acorda_noite ? "amber" : "green"} />
                                                </div>
                                                <DataField label="Horário de Treino" value={standardAnamnesis.horario_treino || "--"} />
                                                <DataField label="Tempo Disp. Treino" value={standardAnamnesis.tempo_disponivel_treino || "--"} />
                                                <DataField label="Dias de Treino/Semana" value={standardAnamnesis.dias_treino_semana ? `${standardAnamnesis.dias_treino_semana} dias` : "--"} />
                                            </ClinicalSection>

                                            <ClinicalSection title="3. Nutrição & Hábitos" icon={Activity}>
                                                <div className="grid grid-cols-2 gap-4 mb-4">
                                                    <div className="flex items-end gap-2">
                                                        <span className="text-3xl font-light text-foreground">{formatNumber(standardAnamnesis.peso)}</span>
                                                        <span className="text-[10px] text-muted-foreground mb-1.5 tracking-widest">KG</span>
                                                    </div>
                                                    <div className="flex items-end gap-2">
                                                        <span className="text-3xl font-light text-foreground">{formatNumber(standardAnamnesis.altura)}</span>
                                                        <span className="text-[10px] text-muted-foreground mb-1.5 tracking-widest">M</span>
                                                    </div>
                                                </div>
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Status do Peso" value={standardAnamnesis.peso_status || "Estável"} />
                                                    <DataField label="Altura" value={standardAnamnesis.altura_status || "Estável"} />
                                                </div>
                                                <DataField label="Alimentos Restritos" value={standardAnamnesis.alimentos_restritos || "Nenhum"} isLong />
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Já fez dieta?" value={standardAnamnesis.ja_fez_dieta ? "Sim" : "Não"} badge={standardAnamnesis.ja_fez_dieta ? "amber" : "green"} />
                                                    <DataField label="Resultado da Dieta" value={standardAnamnesis.resultado_dieta || "--"} />
                                                </div>
                                                <DataField label="Intestino" value={standardAnamnesis.intestino || "--"} badge={standardAnamnesis.intestino === 'Preso' ? 'amber' : 'green'} />
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Dias sem banheiro" value={standardAnamnesis.dias_sem_banheiro || "--"} />
                                                    <DataField label="Vezes ao dia" value={standardAnamnesis.vezes_banheiro_dia || "--"} />
                                                </div>
                                                <DataField label="Ingestão de Água" value={standardAnamnesis.litros_agua_dia ? `${standardAnamnesis.litros_agua_dia}L / dia` : "--"} />
                                                <div className="space-y-3 pt-2">
                                                    <div className="flex justify-between items-center">
                                                        <p className="text-[9px]  text-muted-foreground uppercase tracking-widest">Vontade de Doces</p>
                                                        <span className="text-xs  text-amber-600">{standardAnamnesis.vontade_doce || 0}/10</span>
                                                    </div>
                                                    <div className="h-1.5 w-full bg-muted/30 rounded-full overflow-hidden">
                                                        <div
                                                            className="h-full bg-linear-to-r from-amber-400 to-amber-600"
                                                            style={{ width: `${(standardAnamnesis.vontade_doce || 0) * 10}%` }}
                                                        />
                                                    </div>
                                                </div>
                                                <DataField label="Horários Maior Apetite" value={standardAnamnesis.horarios_maior_apetite || "--"} />
                                                <DataField label="Preferência Lanches" value={standardAnamnesis.preferencia_lanches || "--"} />
                                                <DataField label="Frutas de Preferência" value={standardAnamnesis.frutas_preferencia || "--"} />
                                            </ClinicalSection>

                                            <ClinicalSection title="4. Histórico de Saúde" icon={Heart}>
                                                <DataField label="Doenças Familiares" value={standardAnamnesis.doenca_familiar || "Nenhuma relatada"} isLong />
                                                <DataField label="Problema de Saúde" value={standardAnamnesis.problema_saude ? standardAnamnesis.problemas_saude_detalhes : "Não"} badge={standardAnamnesis.problema_saude ? "red" : "green"} isLong={standardAnamnesis.problema_saude} />
                                                <DataField label="Problema Articular" value={standardAnamnesis.problema_articular || "Não"} />
                                                <DataField label="Uso de Medicamentos" value={standardAnamnesis.uso_medicamentos ? standardAnamnesis.medicamentos_detalhes : "Não"} badge={standardAnamnesis.uso_medicamentos ? "amber" : "green"} isLong={standardAnamnesis.uso_medicamentos} />
                                                <DataField label="Alergia a Medicamento" value={standardAnamnesis.alergia_medicamento || "Nenhuma"} />
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Uso de Cigarros" value={standardAnamnesis.uso_cigarros ? "Sim" : "Não"} badge={standardAnamnesis.uso_cigarros ? "red" : "green"} />
                                                    <DataField label="Intolerâncias" value={standardAnamnesis.intolerancia ? standardAnamnesis.intolerancia_detalhes : "Não"} badge={standardAnamnesis.intolerancia ? "amber" : "green"} />
                                                </div>
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Anticoncepcional" value={standardAnamnesis.uso_anticoncepcional ? "Sim" : "Não"} />
                                                    <DataField label="Uso de Álcool" value={standardAnamnesis.uso_alcool ? standardAnamnesis.alcool_frequencia : "Não"} badge={standardAnamnesis.uso_alcool ? "amber" : "green"} />
                                                </div>
                                                <DataField label="Termogênicos Usados" value={standardAnamnesis.termogenico_usado || "Nenhum"} />
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Já usou Anabolizante?" value={standardAnamnesis.ja_usou_anabolizante ? "Sim" : "Não"} badge={standardAnamnesis.ja_usou_anabolizante ? "red" : "green"} />
                                                    <DataField label="Pretende usar?" value={standardAnamnesis.pretende_usar_anabolizante ? "Sim" : "Não"} badge={standardAnamnesis.pretende_usar_anabolizante ? "amber" : "green"} />
                                                </div>
                                                <DataField label="Histórico Anabolizantes" value={standardAnamnesis.anabolizante_problemas || "Sem registros"} isLong={!!standardAnamnesis.anabolizante_problemas} />
                                            </ClinicalSection>

                                            <ClinicalSection title="5. Objetivos & Compromisso" icon={Target}>
                                                <DataField label="Objetivo Principal" value={standardAnamnesis.objetivo || "Não definido"} isLong />
                                                <div className="grid grid-cols-2 gap-4">
                                                    <DataField label="Meta de Peso" value={standardAnamnesis.target_weight ? `${formatNumber(standardAnamnesis.target_weight)} kg` : "--"} />
                                                    <DataField label="Meta de Gordura" value={standardAnamnesis.target_body_fat ? `${formatNumber(standardAnamnesis.target_body_fat)} %` : "--"} />
                                                </div>
                                                <DataField label="Compromisso c/ Relatórios" value={standardAnamnesis.compromisso_relatorios ? "Comprometido" : "Não comprometido"} badge={standardAnamnesis.compromisso_relatorios ? "green" : "red"} />
                                            </ClinicalSection>

                                            <ClinicalSection title="6. Medidas Antropométricas" icon={Ruler}>
                                                <div className="grid grid-cols-3 gap-4">
                                                    <DataField label="Pescoço" value={standardAnamnesis.pescoco ? `${formatNumber(standardAnamnesis.pescoco)} cm` : "--"} />
                                                    <DataField label="Cintura" value={standardAnamnesis.cintura ? `${formatNumber(standardAnamnesis.cintura)} cm` : "--"} />
                                                    <DataField label="Quadril" value={standardAnamnesis.quadril ? `${formatNumber(standardAnamnesis.quadril)} cm` : "--"} />
                                                </div>
                                            </ClinicalSection>
                                        </div>
                                    </div>
