...
def investigate_chicken_org():
    ...
    print("Investigacao do caso do frango organico")
    print("="*50)
    ...
    print("1. Verificando se o alimento 'frango organico' existe nas bases de dados...")
    ...
    for source, filepath in sources.items():
        try:
            ...
            for item in data:
                ...
                if any(term in name for term in ['frango organico', 'frango organico', 'frango org', 'organic chicken']):
                    print(f"    Encontrado: {item.get('description') or item.get('nome')}")
                    print(f"      Proteina: {protein}")
                    print(f"      Energia: {energy}")
                    print(f"      Lipidios: {lipid}")
    ...
    if not found_anything:
        print("\n  Nenhum alimento com variacao de 'frango organico' encontrado nas bases de dados.")
        print("\n  2. Verificando se ha frango convencional para comparar:")
        ...
        for source, filepath in sources.items():
            try:
                ...
                for sample in chicken_samples:
                    print(f"    - {sample['name']}: Proteina = {sample['protein']}")
                    
            except Exception as e:
                print(f"    Erro ao ler {source}: {e}")
    
    print("\n3. Conclusao:")
    print("   Se o alimento 'frango organico' nao existe nas bases de dados,")
    print("   isso explicaria a ausencia de dados nutricionais.")
    print("   ")
    print("   Se ele existe mas tem proteina zerada, pode ser:")
    print("   - Um erro de importacao dos dados")
    print("   - Uma lacuna nos dados originais da fonte")
    print("   - Um erro de digitacao ou codificacao do nome")
